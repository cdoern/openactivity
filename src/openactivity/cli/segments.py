"""Unified segments commands — provider-agnostic."""
...
segments_app = typer.Typer(
    ...
    help=(
        ...
        "Examples:\n\n"
        "  openactivity segments list\n"
        "  openactivity segment 12345 efforts\n"
        "  openactivity segment 12345 trend\n"
    ),
...
def list_segments(
    ...
    Examples:
        openactivity segments list
        openactivity segments list --type Run
        openactivity segments list --json
    """
    ...
    try:
        ...
        if not segments:
            if use_json:
                ...
                console.print("No starred segments found. Sync your provider data first.")
            return
...
# ── segment app (per-segment commands) ──
...
def show_segment_efforts(
    ...
    Examples:
        openactivity segment 12345 efforts
        openactivity segment 12345 efforts --limit 50
        openactivity segment 12345 efforts --json
    """
    state = get_global_state()
    use_json = state.get("json", False)
...
def show_segment_leaderboard(
    ...
    """View segment leaderboard (requires live Strava API call).

    Examples:
        openactivity segment 12345 leaderboard
        openactivity segment 12345 leaderboard --friends
        openactivity segment 12345 leaderboard --gender M --json
    """
    state = get_global_state()
    use_json = state.get("json", False)
...
def show_segment_trend(
    ...
    Examples:
        openactivity segment 12345 trend
        openactivity segment 12345 trend --json
    """
    state = get_global_state()
    use_json = state.get("json", False)
    units = state.get("units", "metric")
