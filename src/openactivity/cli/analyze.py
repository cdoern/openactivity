"""Unified analyze commands for performance trends across all providers."""
...
app = typer.Typer(
    ...
    help=(
        ...
        "Examples:\n\n"
        "  openactivity analyze summary\n"
        "  openactivity analyze pace --last 90d\n"
        "  openactivity analyze zones --zone-type heartrate\n"
        "  openactivity analyze power-curve\n"
...
def analyze_summary(
    ...
    Examples:
        openactivity analyze summary
        openactivity analyze summary --period monthly --last 1y
        openactivity analyze summary --type Run --json
        openactivity analyze summary --provider strava
    """
...
def analyze_pace(
    ...
    Examples:
        ...
        openactivity analyze pace --last 1y --type Run
        openactivity analyze pace --json
    ...
    try:
        ...
        result = compute_pace_trend(session, last=last, activity_type=activity_type, provider=provider)
...
def analyze_zones(
    ...
    Examples:
        openactivity analyze zones
        openactivity analyze zones --zone-type power --type Ride
        openactivity analyze zones --last 90d --json
    """
    state = get_global_state()
    use_json = state.get("json", False)
...
def analyze_power_curve(
    ...
    Examples:
        openactivity analyze power-curve
        openactivity analyze power-curve --last 1y
        openactivity analyze power-curve --json
    """
    state = get_global_state()
    use_json = state.get("json", False)
...
def analyze_effort(
    ...
    Examples:
        openactivity analyze effort
        openactivity analyze effort --last 6m
        openactivity analyze effort --type Ride --last 1y
        openactivity analyze effort --json
    """
    state = get_global_state()
    use_json = state.get("json", False)
    units = state.get("units", "metric")
...
def analyze_blocks(
    ...
    Examples:
        openactivity analyze blocks
        openactivity analyze blocks --last 1y
        openactivity analyze blocks --type Ride --last all
        openactivity analyze blocks --json
    """
    state = get_global_state()
    use_json = state.get("json", False)
    units = state.get("units", "metric")
...
def analyze_correlate(
    ...
    Examples:
      ...
      openactivity analyze correlate --x weekly_distance --y avg_pace
      openactivity analyze correlate --x weekly_distance --y avg_pace --lag 4
      openactivity analyze correlate --x rest_days --y avg_hr --last 6m
    """
    state = get_global_state()
    use_json = state.get("json", False)
    units = state.get("units", "metric")
...
def analyze_fitness_cmd(
    ...
    Examples:
        ...
        openactivity analyze fitness --provider strava
    """
    state = get_global_state()
    use_json = state.get("json", False)
    ...
    try:
        ...
        result = run_fitness(session, last=last, activity_type=activity_type, provider=provider)
